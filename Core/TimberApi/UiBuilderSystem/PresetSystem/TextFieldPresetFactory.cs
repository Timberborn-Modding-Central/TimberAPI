...
namespace TimberApi.UiBuilderSystem.PresetSystem
{
    public class TextFieldPresetFactory
    {
        ...
        public NineSliceTextField InGameTextField(Length width, Length height = default, bool isMultiLine = false, Length fontSize = default, Color fontColor = default,
            StyleColor backgroundColor = default, string name = null, Action<TextFieldBuilder> builder = default)
        {
            TextFieldBuilder textFieldBuilder = _componentBuilder.CreateTextField().SetName(name).SetWidth(width).SetMultiLine(isMultiLine).SetStyle(style =>
            {
