...
namespace TimberApi.UIBuilderSystem.StyleSheetSystem
{
    public class ComplexSelectorBuilder
    {
        ...
        public void Build()
        {
            if (_parts.Count > 0)
            {
                CreateComplexSelector(StyleSelectorRelationship.None);
            }
            
            if (_styleSelectorRelationship != StyleSelectorRelationship.None && _parts.Count == 0)
            {
                throw new InvalidOperationException($"Can not end with an {_styleSelectorRelationship} selector relationship");
            }
